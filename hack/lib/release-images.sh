...
readonly bin_target=yurt-user-controller
...
# gen_yamls generates yaml files for the yurt-user-controller
gen_yamls() {
    ...
    (
        rm -rf ${BUILD_YAML_DIR}/yurt-user-controller
        cp -rf $YURT_ROOT/config/yurt-user-controller ${BUILD_YAML_DIR}
#        cp -rf $YURT_ROOT/config ${BUILD_YAML_DIR}
        cd ${BUILD_YAML_DIR}/yurt-user-controller/manager
#        cd ${BUILD_YAML_DIR}/config/manager
        kustomize edit set image controller=$REPO/yurt-user-controller:${TAG}
	)
    set +x
    echo "==== create yurt-user-controller.yaml in $OUT_YAML_DIR ===="
    kustomize build ${BUILD_YAML_DIR}/yurt-user-controller/default > ${OUT_YAML_DIR}/yurt-user-controller.yaml
